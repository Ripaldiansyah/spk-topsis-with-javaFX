...
public class calcTopsisViewController implements Initializable {
    ...
    private Map<MFXTextField, VBox> textFieldToVBoxMap = new HashMap<>();
    ...
    private void loadDataCriteria() {
        ...
        String[] dataCriteria = new String[selectedCriteriaList.size()];
        dataCriteria = selectedCriteriaList.toArray(dataCriteria);
    ...
    private void dataCriteriaType() {
        if (topsisModel.getTypeCriteria().equalsIgnoreCase("Benefit")) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                textFieldToVBoxMap.put(ratingField, criteriaVbox);
    ...
    // int i=0;
    // for(Node node : criteriaVbox.getChildren()){
    // String textValue = "";
    // int size = ratingFieldData.size();
    // if (node instanceof Rating) {
    // double ratingValue = ((Rating) node).getRating();
    // textValue = String.valueOf(ratingValue);
    // }else{
    // textValue = ((MFXTextField) node).getText().trim();
